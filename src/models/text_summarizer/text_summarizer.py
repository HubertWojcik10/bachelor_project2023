...
class TextSummarizer(Model):
    ...
    def _add_pad_token_to_text(self, text: str, max_length: int=254):
        """
            Add the pad token to the text
        """
        return text + " <pad>" * (max_length - len(text.split()))

    def summarize(self, df):
        """
            Summarize the text1 and text2 columns (used only once in our case as we save the summarized data to csv)
        """
    ...
    def run(self, train: bool = True) -> None:
        """
            Run the model
        """
        train_data, test_data = self.get_data(self.params_dict["summary_train_data_path"], self.params_dict["summary_test_data_path"], self.dev)
        if train:
            ...
            dev_true, dev_pred, cur_pearson = self.predict(test_loader, model, validation=False)